...
const GoalsForm = ({history, setHistory, editingGoal, setEditingGoal}) => {
    ...
    // Modify the handleSave wrapper
    const handleSaveWithReminders = async () => {
        const saveResult = await handleSave();
        if (saveResult) {
            // Goal was saved successfully
            setSavedGoal({ ...goalData, title: getFinalTitle() });
    ...
    return (
        <div className={`goalsform-container ${showReminderCard ? 'with-reminder' : ''}`}>
            <div className="goalsform-main">
            ...
            {/* Side-by-side Reminder Card */}
            <ReminderCard
                show={showReminderCard}
                goal={savedGoal}
                onClose={() => setShowReminderCard(false)}
                onReminderSet={(reminders) => {
